...
def account_reset(settings):
    ...
    db = get_db()
    db.execute("UPDATE user SET password = ? WHERE username = ?", (hashed_passw, uname))
    db.commit()
...
def get_settings(settings_file="settings.ini"):
    ...
    if not len(pw) == 102 and pw != "new_account_password":
...
def set_settings(settings, settings_file="settings.ini"):
    ...
    for key, value in setting.items():
        if key == "register":
            ...
            try:
                ...
                flash(
                    "Use Copyright Start Date must be True or False: " + str(err),
                    "error",
                )
                error = True
    ...
    with open(
        os.path.join(app.instance_path, settings_file), "w", encoding="utf-8"
    ) as setting:
